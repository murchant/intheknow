...
def make_commands():
    ...
    for i in player_rels:
        for j in i:
            term = '"' + j + '"'
            cmd = "python GetOldTweets/Exporter.py --querysearch " + term + " --maxtweets 100 --since 2018-06-01 --until 2018-08-30  --output info/true.csv + &"
            file.write(cmd+"\n")
    ...
    for i in player_rels:
        for j in i:
            term = '"' + j + '"'
            cmd = "python GetOldTweets/Exporter.py --querysearch " + term + " --maxtweets 100 --since 2018-06-01 --until 2018-08-30  --output info/trueShorter.csv + &"
            file.write(cmd+"\n")
            wait_break=wait_break+1
            if wait_break%20==0:
                file.write("wait"+"\n")
